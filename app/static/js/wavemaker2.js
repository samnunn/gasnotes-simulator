...
customElements.define(
    ...
    class extends HTMLElement {
        ...
        constructor() {
            ...
            this.morphology = this.getAttribute("sim-value") || "sinus";
            ...
            this.fillOpacity =
                parseFloat(this.getAttribute("fill-opacity")) || 1.0;
...
path {
    ...
    padding: 0;
        ...
        beat(mandatoryWidth = 0, repeating = false, animating = true) {
            ...
            if (this.morphology != this.last_morphology) {
                ...
                newPath.setAttribute(
                    "d",
                    `M ${targetWave.x_cursor},${this.y_cursor} `,
                );
            ...
            if (this.morphology == "sim-disconnect") {
                this.frontWave
                    ...
                    .setAttribute("stroke-dasharray", "20,20");
            }
            ...
            if (overshoot >= 0) {
                ...
                this.frontWave.animate(
                    [{ borderWidth: "0px" }, { borderWidth: "20px" }],
                    {
                        duration: 100,
                        iterations: 1,
                        easing: "linear",
        ...
        insertNextComplex(targetWave = this.frontWave, mandatoryWidth = 0) {
            ...
            try {
                draftKeyframes =
                    this.waveGenerators[this.morphology](mandatoryDuration);
            } catch (error) {
                console.error(
                    `An invalid wave morphology (${this.morphology}) was provided to wavemaker!`,
                );
                console.error(error);
                draftKeyframes =
                    this.waveGenerators["flatline"](mandatoryDuration);
            }

            // squash to mandatoryWidth
            if (mandatoryWidth != 0) {
                let initialDuration =
                    this.getDurationOfKeyframes(draftKeyframes);
        ...
        waveGenerators = {
            artline: (targetDuration) => {
                ...
                let pulseWidth = Math.min(0.3 * targetDuration, 300);

                // MAKE QRS
                let keyframes = [
                    [0, 0, 5], // start
                    [pulseWidth, 0.7, notchWidth * -0.1], // peak
            ...
            "artline-underdamped": (targetDuration) => {
                ...
                let notchWidth = Math.min(0.15 * targetDuration, 100);
                let pulseWidth = Math.min(0.3 * targetDuration, 200);
                ...
                keyframes[keyframes.length] = [
                    targetDuration - measuredDuration,
                    0,
                    -0.1 * (targetDuration - measuredDuration),
                ];
            ...
            cpr: (targetDuration) => {
                ...
                if (targetDuration == 0) {
                    ...
                    targetDuration = this.randomNumberBetween(
                        1.2 * idealDuration,
                        0.9 * idealDuration,
                    );
                }
                ...
                let keyframes = [
                    [0, randomBaseline, 3],
                    [
                        targetDuration * this.randomNumberBetween(0.2, 0.05),
                        randomPeak,
                    ],
                    [targetDuration * 0.05, randomPeak * 1.1, -1],
                    [
                        targetDuration * randomPeriod,
                        randomPeak * this.randomNumberBetween(1.2, 1),
                        -7,
                        2,
                    ],
                    ...
                    [
                        targetDuration * this.randomNumberBetween(0.3, 0.1),
                        randomBaseline,
                        -2,
                        -4,
                    ],
                ...
                if (Math.random() > 0.85) {
                    ...
                    keyframes = keyframes.concat([
                        ...
                        [
                            targetDuration * (Math.random() * 1.5 + 1.5),
                            randomY,
                            -5,
                            2,
                        ],
            ...
            spo2: (targetDuration) => {
                ...
                let keyframes = [
                    [0, 0, 4, -4],
                    [
                        this.interpolateDuration(180, 100, this.rate),
                        this.interpolateDuration(0.5, 0.4, this.rate) +
                            0.1 * Math.random(),
                        this.interpolateDuration(-6, -4, this.rate),
                        5,
                    ],
                    [
                        this.interpolateDuration(220, 120, this.rate),
                        0.15,
                        this.interpolateDuration(-10, -7, this.rate),
                        this.interpolateDuration(15, 20, this.rate),
                    ],
                ];
            ...
            "spo2-badtrace": (targetDuration) => {
                ...
                let keyframes = [
                    [0, 0, -wobble],
                    [
                        0.45 * (60000 / this.rate),
                        0.1 * this.interpolateDuration(0.7, 0.2, this.rate) +
                            0.1 * Math.random(),
                        wobble,
                        0,
                    ],
                    ...
                    for (let i = 0; i < fibCount; i++) {
                        let newFrame = [
                            this.randomNumberBetween(
                                1.5 * baseFibrillationWaveWidth,
                                0.5 * baseFibrillationWaveWidth,
                            ),
            ...
            vfib: (targetDuration) => {
                ...
                let wanderingBaseline =
                    (0.3 * Math.sin(this.complexCount * 0.05)) / Math.PI;
            ...
            "vtach-monomorphic": (targetDuration) => {
                ...
                let wanderingBaseline =
                    (0.05 * Math.sin(this.complexCount * 0.3)) / Math.PI;
            ...
            "vtach-torsades": (targetDuration) => {
                ...
                let randomCompoent = Math.random() * 0.2;
                let wanderingBaseline =
                    (0.2 * Math.sin(this.complexCount * 0.3)) / Math.PI;

                let isoelectricLine =
                    0 -
                    (0.5 * Math.sin(this.complexCount * 0.2)) / Math.PI -
                    randomCompoent +
                    wanderingBaseline;
            ...
            sinus: (targetDuration) => {
                ...
                let keyframes = [
                    ...
                    [
                        this.interpolateDuration(100, 80, this.rate),
                        0.1,
                        this.interpolateDuration(-4, -2, this.rate),
                    ], // p wave (peak)
                    [
                        this.interpolateDuration(100, 80, this.rate),
                        0,
                        this.interpolateDuration(-4, 0, this.rate),
                        0,
                    ], // pr segment start (isoelectric)
                    [
                        this.interpolateDuration(80, 0, this.rate),
                        0,
                        this.interpolateDuration(-2, 0, this.rate),
                        0,
                    ], // end of pr segment (isoelectric)
                    [
                        this.interpolateDuration(40, 25, this.rate),
                        this.randomNumberBetween(-0.07, -0.04),
                        -0.05,
                    ], // q wave (trough)
                    [
                        this.interpolateDuration(50, 25, this.rate),
                        this.randomNumberBetween(1, 0.9),
                        -0.05,
                    ], // r wave (peak)
                    [
                        this.interpolateDuration(50, 25, this.rate),
                        this.randomNumberBetween(-0.1, -0.05),
                        -0.05,
                    ], // s wave (trough)
                    [
                        this.interpolateDuration(70, 30, this.rate),
                        0,
                        this.interpolateDuration(-5, 0, this.rate),
                        5,
                    ], // st segment
                    [
                        this.interpolateDuration(180, 60, this.rate),
                        0.175,
                        this.interpolateDuration(-6, -2, this.rate),
                    ], // t wave (peak)
                    [
                        this.interpolateDuration(180, 60, this.rate),
                        isoelectricLine,
                        this.interpolateDuration(-8, 0, this.rate),
                    ], // qt (isoelectric)
                ];
            ...
            avnrt: (targetDuration) => {
                ...
                let keyframes = [
                    ...
                    [
                        this.interpolateDuration(170, 130, this.rate),
                        isoelectricLine,
                        0,
                        0,
                    ],
            ...
            hb2m1: (targetDuration) => {
                ...
                let keyframes = [
                    ...
                    [
                        0,
                        isoelectricLine,
                        this.interpolateDuration(5, 0.1, this.rate),
                        this.interpolateDuration(0, 2, this.rate),
                    ], // start (isoelectric)
                    [
                        this.interpolateDuration(100, 80, this.rate),
                        0.1,
                    ...
                    [
                        this.interpolateDuration(
                            (this.complexCount % 4) * 80 + 80,
                            0,
                            this.rate,
                        ),
                        0,
                    ...
                    [
                        this.interpolateDuration(50, 25, this.rate),
                        skippedBeat * this.randomNumberBetween(1, 0.9),
                        -0.05,
                    ], // r wave (peak)
                    [
                        this.interpolateDuration(50, 25, this.rate),
